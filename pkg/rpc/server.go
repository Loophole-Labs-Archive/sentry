...
func (s *Server) DoRPC(request *Request, response *Response) (err error) {
	...
	s.writeQueue <- inflightRequest

	timeout := time.NewTimer(Timeout)

	select {
	case <-s.ctx.Done():
		s.logger.Warnf("context was canceled, cancelling request %s of type %d\n", inflightRequest.Request.UUID, inflightRequest.Request.Type)
		err = context.Canceled
	case <-inflightRequest.complete:
		s.logger.Infof("request %s of type %d was completed\n", inflightRequest.Request.UUID, inflightRequest.Request.Type)
		err = nil
	case <-timeout.C:
		s.logger.Warnf("request %s of type %d timed out\n", inflightRequest.Request.UUID, inflightRequest.Request.Type)
...
func (s *Server) write() {
	...
	for {
		...
		s.logger.Infof("writing request %s of type %d (priority %t)\n", inflightRequest.Request.UUID, inflightRequest.Request.Type, priority)
		_, err := s.activeConn.Write(inflightRequest.RequestBuffer.Bytes())
		if err != nil {
			s.logger.Errorf("unable to write request: %v\n", err)
			select {
			case s.priorityWriteQueue <- inflightRequest:
				s.logger.Infof("requeueing request %s of type %d for priority writing\n", inflightRequest.Request.UUID, inflightRequest.Request.Type)
			default:
				s.logger.Warnf("priority write queue is full, dropping request %s of type %d\n", inflightRequest.Request.UUID, inflightRequest.Request.Type)
			}
			break
		}
	}
...
func (s *Server) read() {
	...
	for {
		...
		if err != nil {
			s.logger.Errorf("unable to read from connection: %v\n", err)
			goto OUT
		}
		_uuid, err = DecodeUUID(buf[:n])
		if err != nil {
			s.logger.Errorf("unable to decode UUID: %v\n", err)
			continue
		}
